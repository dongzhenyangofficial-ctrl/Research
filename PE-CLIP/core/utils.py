...
def process_data_item(opt, data_item):
    visual, target, audio, visualization_item, mask = data_item
    # target = target.cuda()

    # visual = visual.cuda()
    # audio = audio.cuda()
    # assert visual.shape[0] == audio.shape[0]
    batch = visual.shape[0]
    return visual, target, audio, visualization_item, mask, batch


def run_model(opt, inputs, model, criterion, i=0, print_attention=False, period=30, return_attention=False):
    visual, target, audio, bool_masked_pos = inputs
    output, F, T, A = model(visual, target)
    loss = criterion(output, F, T, A, target)
    return output, loss

def calculate_accuracy(outputs, targets):
    batch_size = targets.shape[0]
    values, indices = outputs.topk(k=1, axis=1, largest=True)
    pred = indices
    pred = pred.t()
    correct = paddle.equal(pred, targets.reshape([1, -1]))
    n_correct_elements = paddle.cast(correct, dtype='float32')
