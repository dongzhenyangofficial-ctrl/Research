...
class VE8Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        self.spatial_transform.randomize_parameters(snippet[0])
        snippets_transformed = []
        for snippet in snippets:
            snippet = [self.spatial_transform(img) for img in snippet]
            snippet = paddle.stack(snippet, 0)
            snippets_transformed.append(snippet)
        snippets = snippets_transformed
        snippets = paddle.stack(snippets, 0)
        seq_len, snippet_duration, nc, sample_size, _ = snippets.shape
        snippets = snippets.reshape([seq_len * snippet_duration, nc, sample_size, sample_size])
        # with paddle.no_grad():
        #     snippets = clip.encode_image(snippets)
        # nc = snippets.shape[1]
        
        target = self.target_transform(data_item)
        visualization_item = [data_item['video_id']]

        return snippets, target, audios, visualization_item, self.masking_generator()
